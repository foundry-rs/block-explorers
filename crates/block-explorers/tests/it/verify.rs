...
async fn can_verify_contract_json_multi_file_v2_fails_base() {
    ...
    let contract_json_string = serde_json::to_string(&contract.expect("Contract did not compile to standard json")).expect("Did not serialize JSON for verification");

    trace!("{:?}", contract_json_string.clone());

    let contract =
        VerifyContract::new(address, contract_name, contract_json_string, compiler_version.to_string())
